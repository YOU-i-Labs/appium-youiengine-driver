...
class YouiDriver extends BaseDriver {
  ...
  async setupNewIOSDriver (opts) {
    ...
    let caps = {
      ...
      // Disabling the proxy CommandTimeout in the iOS driver since we are now handling it in the Youi Driver
      newCommandTimeout: 0
    };
  ...
  async setupNewAndroidDriver (opts) {
    ...
    let caps = {
      ...
      deviceName: opts.deviceName,
      avd: this.caps.avd,
      // Disabling the proxy CommandTimeout in the Android driver since we are now handling it in the Youi Driver
      newCommandTimeout: 0
    };
