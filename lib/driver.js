...
class YouiEngineDriver extends BaseDriver {
  ...
  startBlueSkySession (caps) {
    ...
    this.validateAppLocation(caps.app);
    let shell = require('shelljs');
    let bluesky_install_script = "curl -v -# -f -i --user '" + caps.username + ":" + caps.password + "' --digest --progress-bar -F 'mysubmit=Install' -F 'archive=@" + caps.app + "' -F 'passwd=' http://" + caps.youiEngineAppAddress + "/plugin_install | grep '<font color' | sed 's/<font color=\'red\'>//' "; // eslint-disable-line no-useless-escape
